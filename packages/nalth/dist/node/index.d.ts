...
import { D as FetchFunctionOptions, O as FetchResult, c as ModuleEvaluator, f as ModuleRunnerOptions, h as ModuleRunner, s as FetchFunction, u as ModuleRunnerHmr, w as ModuleRunnerTransport } from "./index-B2tuDhTa.js";
...
//#endregion
//#region src/node/security.config.d.ts
 ...
 */
declare function createSecurityMiddleware(config?: Partial<SecurityConfig>): Connect.NextHandleFunction;
/**
 * Generate SRI hash for content
 */
declare function generateSRIHash(content: string, algorithm?: 'sha256' | 'sha384' | 'sha512'): string;
/**
 * Audit code for security violations
 */
declare function auditCode(code: string, patterns: string[]): {
  violations: string[];
  safe: boolean;
};
//#endregion
//#region src/node/plugins/security.d.ts
interface SecurityPluginOptions extends Partial<SecurityConfig> {}
/**
 * Nalth Security Plugin - Provides SRI, security auditing, and secure build features
 */
declare function securityPlugin(options?: SecurityPluginOptions): Plugin;
/**
...
declare function vue(options?: VuePluginOptions): Plugin;
declare namespace index_d_exports {
  export { createPluginHookUtils, getCachedFilterForPlugin, getHookHandler, getSortedPluginsByHook, resolvePlugins, viteAliasCustomResolver, vue };
}
declare function resolvePlugins(config: ResolvedConfig, prePlugins: Plugin[], normalPlugins: Plugin[], postPlugins: Plugin[]): Promise<Plugin[]>;
declare function createPluginHookUtils(plugins: readonly Plugin[]): PluginHookUtils;
declare function getSortedPluginsByHook<K$1 extends keyof Plugin>(hookName: K$1, plugins: readonly Plugin[]): PluginWithRequiredHook<K$1>[];
declare function getHookHandler<T extends ObjectHook<Function>>(hook: T): HookHandler<T>;
...
declare function getCachedFilterForPlugin<H extends 'resolveId' | 'load' | 'transform'>(plugin: Plugin, hookName: H): FilterForPluginValue[H] | undefined;
declare const viteAliasCustomResolver: ResolverFunction$1;
//#endregion
//#region src/node/https-security.d.ts
interface NalthHttpsConfig extends HttpsServerOptions {
  /** Auto-generate self-signed certificates for development */
  autoGenerate?: boolean;
 ...
 */
declare function generateSelfSignedCert(certDir: string, logger: Logger): {
  cert: string;
  key: string;
};
/**
 ...
 */
declare function resolveNalthHttpsConfig(https: NalthHttpsConfig | boolean | undefined, cacheDir: string, logger: Logger): Promise<HttpsServerOptions | undefined>;
//#endregion
